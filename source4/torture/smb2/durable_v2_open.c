...
/**
 * persistent handle variant of reopen2
 * basic test for doing a durable open
 * tcp disconnect, reconnect, do a durable reopen (succeeds)
 */
bool test_durable_v2_open_reopen2_persistent(struct torture_context *tctx,
					   struct smb2_tree *tree)
{
	NTSTATUS status;
	TALLOC_CTX *mem_ctx = talloc_new(tctx);
...
struct torture_suite *torture_smb2_durable_v2_open_init(TALLOC_CTX *ctx)
{
	...
	torture_suite_add_1smb2_test(suite, "reopen2-persistent", test_durable_v2_open_reopen2_persistent);
