...
/**
 * lease/persistent variant of reopen2
 * basic test for doing a durable open
 * tcp disconnect, reconnect, do a durable reopen (succeeds)
 */
bool test_durable_v2_open_reopen2_lease_ca(struct torture_context *tctx,
					   struct smb2_tree *tree)
{
	NTSTATUS status;
	TALLOC_CTX *mem_ctx = talloc_new(tctx);
	...
	ssn_caps = smb2cli_conn_server_capabilities(tree->session->transport->conn);
	if (!(ssn_caps & SMB2_CAP_LEASING)) {
		torture_skip(tctx, "leases are not supported");
	}

	share_caps = smb2cli_tcon_capabilities(tree->smbXcli);
	if (!(share_caps & SMB2_SHARE_CAP_CONTINUOUS_AVAILABILITY)) {
		torture_skip(tctx, "persistent opens not supported");
	}
	...
	CHECK_VAL(io.out.durable_open, false);
	CHECK_VAL(io.out.durable_open_v2, false); /* no dh2q response blob */
	CHECK_VAL(io.out.persistent_open, false); /* no dh2q response blob */
	CHECK_VAL(io.out.oplock_level, SMB2_OPLOCK_LEVEL_LEASE);
	CHECK_VAL(io.out.lease_response_v2.lease_key.data[0], lease_key);
	CHECK_VAL(io.out.lease_response_v2.lease_key.data[1], ~lease_key);
	CHECK_VAL(io.out.lease_response_v2.lease_state,
...
/**
 * lease/persistent variant of reopen2
 * basic test for doing a durable open of a directory
 * tcp disconnect, reconnect, do a durable reopen (succeeds)
 */
bool test_durable_v2_open_reopen2_lease_cadir(struct torture_context *tctx,
					   struct smb2_tree *tree)
{
	NTSTATUS status;
	TALLOC_CTX *mem_ctx = talloc_new(tctx);
	...
	lease_key = random();
	smb2_lease_v2_create(&io1, &ls, true /* dir */, fname,
			     lease_key, 0, /* parent lease key */
			     smb2_util_lease_state("RWH"), 0 /* lease epoch */);
	...
	h = &_h;
	CHECK_CREATED(&io1, CREATED, FILE_ATTRIBUTE_DIRECTORY);
	CHECK_VAL(io1.out.durable_open, false);
	CHECK_VAL(io1.out.durable_open_v2, true);
	CHECK_VAL(io1.out.persistent_open, true);
	if (io1.out.oplock_level == SMB2_OPLOCK_LEVEL_LEASE) {
		CHECK_VAL(io1.out.lease_response_v2.lease_key.data[0], lease_key);
		CHECK_VAL(io1.out.lease_response_v2.lease_key.data[1], ~lease_key);
	}
	...
	io2.in.persistent_open = true;
	io2.in.create_guid = create_guid;
	/* If the original open got a lease, reconnect with same. */
	if (io1.out.oplock_level == SMB2_OPLOCK_LEVEL_LEASE) {
		io2.in.oplock_level = SMB2_OPLOCK_LEVEL_LEASE;
		io2.in.lease_request_v2 = &ls;
	}
	...
	CHECK_CREATED(&io2, EXISTED, FILE_ATTRIBUTE_DIRECTORY);
	CHECK_VAL(io2.out.durable_open, false);
	CHECK_VAL(io2.out.durable_open_v2, false); /* no dh2q response blob */
	CHECK_VAL(io2.out.persistent_open, false); /* no dh2q response blob */
	/* If the original open got a lease, it should be here too. */
	if (io1.out.oplock_level == SMB2_OPLOCK_LEVEL_LEASE) {
		CHECK_VAL(io2.out.oplock_level, SMB2_OPLOCK_LEVEL_LEASE);
		CHECK_VAL(io2.out.lease_response_v2.lease_key.data[0], lease_key);
		CHECK_VAL(io2.out.lease_response_v2.lease_key.data[1], ~lease_key);
	}
...
struct torture_suite *torture_smb2_durable_v2_open_init(TALLOC_CTX *ctx)
{
	...
	torture_suite_add_1smb2_test(suite, "reopen2-lease-ca", test_durable_v2_open_reopen2_lease_ca);
	torture_suite_add_1smb2_test(suite, "reopen2-lease-cadir", test_durable_v2_open_reopen2_lease_cadir);
