...
static NTSTATUS print_blob(TALLOC_CTX *mem_ctx,
				void *ntacl)
{
  char tab[5*128+1];
  int j;
  ...
  fprintf(stderr,"bloblength : %d\n",blob.length);
  for(j=0;j<blob.length;j++)
  {
    snprintf(tab+(5*j),6,"0x%02hhx,",blob.data[j]);
  }
  tab[5*j]='\0';
  fprintf(stdout,"%s\n",tab);
...
int main(int argc, const char *argv[])
{
	...
	if ( !acl ) {
	  fprintf(stderr,"ACL to be parsed is missing !\n");
	  exit(1);
	}
	...
	status = print_blob(NULL, ntacl);
	if (!NT_STATUS_IS_OK(status)) {
		fprintf(stderr, "print_blob failed: %s\n", nt_errstr(status));
		return 1;
	}
